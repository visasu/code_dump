...
int main(){
	...
	for(uint i=0;i<RowX;i++){
		for(uint j=0;j<ColX;j++){
			X[i][j]=(i*ColX+j+1);
		}
	}
	for(uint i=0;i<RowY;i++){
		for(uint j=0;j<ColY;j++){
			Y[i][j]=(i*ColY+j+1);
		}
	}
	for(uint i=0;i<RowZ;i++){
		for(uint j=0;j<ColZ;j++){
			Z[i][j]=0;
		}
	}
	for(uint i=0;i<ColX;i++){
		temp[i]=0;
	}
	...
	cl::Device default_device=all_devices.at(1);
//	cout<<"Using Device:"<<default_device.getInfo<CL_DEVICE_NAME>()<<"\n";
	...
	cl::Buffer buffer_temp(context,CL_MEM_READ_WRITE, sizeof(temp));
...
*/	
	...
	queue.enqueueWriteBuffer(buffer_temp,CL_TRUE | CL_QUEUE_PROFILING_ENABLE,0,sizeof(temp),&temp);
...
*/	
	...
	queue.enqueueNDRangeKernel(kernel,cl::NullRange,cl::NDRange(RowZ,ColZ,ColX),cl::NDRange(1,1,ColX));
	queue.enqueueBarrierWithWaitList();
